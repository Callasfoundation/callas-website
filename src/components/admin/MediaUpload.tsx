...
export function MediaUpload({
  ...
  return (
    <div>
      ...
      {value && accept === "image" && (
        <img src={value} alt="" className="mt-2 h-28 w-28 object-cover rounded-lg border border-slate-200" />
      )}
      {value && accept === "video" && (
        <video src={value} controls className="mt-2 h-28 rounded-lg border border-slate-200" />
      )}
      <input
        type="url"
        value={value}
        ...
        className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-1.5 text-xs text-muted-foreground focus:outline-none focus:border-brand-blue"
      />
    </div>
  );
}
