...
type Volunteer = { id: string; name: string; email: string; phone: string; track: string; availableDays: string; note: string };
            ...
            <div key={v.id} className="rounded-xl border border-slate-200 p-4">
              <div className="flex justify-between">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">{v.phone} · Available: {v.availableDays}</div>
                </div>
                <button onClick={() => remove(v.id)} className="text-brand-red hover:text-brand-red-dark shrink-0"><Trash2 className="h-4 w-4" /></button>
              </div>
