...
type Resource = { id: number; title: string; type: string; description: string; url: string };
...
function ResourcesPage() {
  ...
  useEffect(() => {
    api.list<Resource>("resources").then(setResources).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <>
      ...
      <section className="bg-white"><div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-16 grid gap-4 md:grid-cols-2">
        ...
        {!loading && resources.length === 0 && <p className="text-muted-foreground col-span-full">No resources added yet.</p>}
        {resources.map((r, i) => (
          <Reveal key={r.id} delay={i * 0.04}>
            <a href={r.url} target="_blank" rel="noopener noreferrer" className="group flex items-start gap-4 rounded-2xl border border-slate-200 bg-white p-6 hover:-translate-y-1 hover:shadow-xl transition-all">
